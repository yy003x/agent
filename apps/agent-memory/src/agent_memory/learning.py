...
用法：
  python apps/agent-memory/bin/agent-learning-review [--days N]   默认 7
  python apps/agent-memory/bin/agent-learning-review --dry-run     打印候选不写文件
  python apps/agent-memory/bin/agent-learning-review promote --file <candidates.md> --candidate N \
    ...
    for q, recs in low.items():
        if len(recs) >= 2:
            cands.append({
                ...
                "target": "apps/content-runtime/src/agent_content_runtime/cli.py",
                "key": f"low-hit:{q}",
            })
