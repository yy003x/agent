...
把「本轮发生了什么」沉淀为 session 记录，供自我进化学习工具使用。

用法：
  python apps/agent-memory/bin/finalize record [--skill <name>] [--status success|partial|failed]
                                    [--summary "<一句话摘要>"] [--handoff]
  python apps/agent-memory/bin/finalize hook     # Stop hook 兜底：stdout 输出 hook JSON
  python apps/agent-memory/bin/finalize mark     # 运行时写操作标记，供 hook 兜底消费
  python apps/agent-memory/bin/finalize snapshot   # 读 git status/diff，判定状态，输出 JSON
...
本脚本不依赖第三方库，可被 Python runtime、Codex/tmux runtime 或 Stop hook 直接调用。
"""
