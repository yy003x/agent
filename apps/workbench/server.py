#!/usr/bin/env python3
"""Local Web UI for the book-operations Agent workbench.

This server intentionally uses the Python standard library for the first GUI
slice so the workbench can start before optional web dependencies are installed.
"""
...
class WorkbenchHandler(SimpleHTTPRequestHandler):
    server_version = "BookAgentWorkbench/0.1"

    def log_message(self, fmt: str, *args) -> None:  # noqa: A003
        sys.stderr.write(f"[workbench] {self.address_string()} {fmt % args}\n")
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if path == "/api/health":
                return self._send_json(health.collect_health())
            if path == "/api/state":
            ...
            if path == "/api/outputs":
                return self._send_json(file_browser.list_outputs())
            if path == "/api/files":
                rel_path = qs.get("path", ["outputs"])[0]
                return self._send_json(file_browser.read_file(rel_path) if not qs.get("list") else file_browser.list_dir(rel_path))
            if path == "/api/kb/search":
                query = qs.get("query", [""])[0]
                modality = qs.get("modality", ["all"])[0]
                topk = int(qs.get("topk", ["10"])[0])
                return self._send_json(kb_search(query, modality, topk))
            if path == "/api/chat/sessions":
                return self._send_json({"sessions": list_sessions()})
            if path.startswith("/api/chat/sessions/") and path.endswith("/operator"):
                session_id = path.split("/")[-2]
                return self._send_json(operator_view(session_id))
            if path.startswith("/api/chat/sessions/") and path.endswith("/runtime"):
                session_id = path.split("/")[-2]
                return self._send_json(session_runtime_status(session_id))
            if path.startswith("/api/chat/sessions/") and path.endswith("/runtime/logs"):
                session_id = path.split("/")[-3]
                max_bytes = int(qs.get("max_bytes", ["40000"])[0])
                return self._send_json(session_runtime_logs(session_id, max_bytes=max_bytes))
            if path.startswith("/api/chat/sessions/"):
                session_id = unquote(path.rsplit("/", 1)[-1])
                return self._send_json(get_session(session_id))
            if path == "/api/runtime/tmux/runs":
                return self._send_json({"runs": MAIN_RUNTIME.list_runs()})
            if path.startswith("/api/runtime/tmux/runs/"):
                parts = path.split("/")
                run_id = parts[5] if len(parts) > 5 else ""
                if len(parts) == 7 and parts[6] == "logs":
                    return self._send_json(MAIN_RUNTIME.run_logs(run_id))
                return self._send_json(MAIN_RUNTIME.run_status(run_id))
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            body = self._body()
            if path == "/api/chat/sessions":
                return self._send_json(create_session(body.get("title", ""), body.get("runtime")))
            if path == "/api/chat/sessions/delete":
                return self._send_json(delete_sessions(body.get("session_ids", [])))
            if path.startswith("/api/chat/sessions/") and path.endswith("/messages"):
                session_id = path.split("/")[-2]
                wait_seconds = float(body.get("wait_seconds", 0))
                return self._send_json(add_message(session_id, body.get("content", ""), wait_seconds))
            if path == "/api/config/runtime":
                return self._send_json(save_workbench_config(body))
            if path == "/api/files/open":
                return self._send_json(open_allowed_path(body.get("path", "")))
            if path == "/api/content/draft":
                return self._send_json(_draft_preview(
                    body.get("brief", ""),
            ...
            if path == "/api/runtime/tmux/runs":
                config = workbench_config()
                runtime = _valid_runtime(body.get("runtime") or config["runtime_provider"], config["runtime_provider"])
                return self._send_json(MAIN_RUNTIME.start_run(
                    runtime,
                    body.get("prompt", "请输出一个简短状态。"),
                    body.get("command") or _command_for_runtime(config, runtime),
                    int(body.get("timeout_seconds", 1800)),
                    _runtime_options_from_config(config),
                ))
            if path.startswith("/api/runtime/tmux/runs/") and path.endswith("/send"):
                run_id = path.split("/")[-2]
                return self._send_json(MAIN_RUNTIME.send_to_run(run_id, body.get("text", "")))
            if path.startswith("/api/runtime/tmux/runs/") and path.endswith("/stop"):
                run_id = path.split("/")[-2]
                return self._send_json(MAIN_RUNTIME.stop_run(run_id))
            if path.startswith("/api/chat/sessions/") and path.endswith("/runtime/stop"):
                session_id = path.split("/")[-3]
                return self._send_json(stop_session_runtime(session_id))
            return self._send_error_json("not found", HTTPStatus.NOT_FOUND)
        except Exception as exc:  # noqa: BLE001
            return self._send_error_json(str(exc), HTTPStatus.BAD_REQUEST)
    ...
    def do_DELETE(self) -> None:  # noqa: N802
        ...
        try:
            if path.startswith("/api/chat/sessions/"):
                session_id = unquote(path.rsplit("/", 1)[-1])
                return self._send_json(delete_session(session_id))
            return self._send_error_json("not found", HTTPStatus.NOT_FOUND)
        except FileNotFoundError as exc:
            return self._send_error_json(str(exc), HTTPStatus.NOT_FOUND)
        except Exception as exc:  # noqa: BLE001
            return self._send_error_json(str(exc), HTTPStatus.BAD_REQUEST)
