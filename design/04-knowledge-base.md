...
> content-runtime 的 `kb` 子命令是 KB 的唯一对外接口；其余层（路由 / skill / workbench-finalizer / 自学习）只通过 `kb search` 消费 KB，不关心其内部实现。
...
| `caption` | string | 图片/视频的 tmux CLI runtime caption 或手工 caption；文档为首 200 字 |
...
多模态解析流程（文档分块 / 图片 runtime caption / 视频抽帧 caption）**与 03-content-agent.md 一致**，本文只描述「落库」差异：
...
- caption 任务走配置的智能 runtime：工作台默认使用 tmux CLI result-file 契约，`codex_cli` 默认、`claude_cli` 通过配置启用，不走 `codex exec` / `claude -p`。当前 runtime 不支持媒体理解时，允许手工 caption 或仅使用文件名/目录标签降级。
...
千级素材直接重跑 ingest 最简单；图片/视频 caption 若已经生成，应使用 caption cache 避免重复启动 caption 任务。
