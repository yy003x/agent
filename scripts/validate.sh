...
check "agentrun provider 配置存在" "test -f config/agentrun/api.yaml && test -f config/agentrun/cli.yaml && test -f config/agentrun/tmux.yaml"
check "agentrun 配置目录只保留 provider 文件" "[ \"$(find config/agentrun -type f | sort | tr '\n' ' ')\" = \"config/agentrun/api.yaml config/agentrun/cli.yaml config/agentrun/tmux.yaml \" ]"
